...
public class Exam_InnerClassListener extends JFrame{
	...
	private class MyActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			// Exam_IndepClassListener 클래스에 있는 setTitle() 메소드를 불러와서 btn.getText() 내용을 바꿔주겠다.
			// 버튼이 눌릴때마다 타이틀이 기존에 설정한 Action 이벤트 리스터 예제 와
			// 번갈아가면서 바뀐다.
			// setTitle 은 Exam_IndepClassListener 클래스 내에 있는 메소드이기 때문에 외부클래스에선 사용할 수 없어서
			// 내부 클래스로만들었을 때에만 이벤트로 변경할 수 있다.
